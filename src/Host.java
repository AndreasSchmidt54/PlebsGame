...
public class Host implements Runnable, ActionListener {
	...
	public void run() {
		...
		try {
			while ((message = reader.readLine()) != null) {
				...
				// Dem Host wird die Spielernummer 1 zugeordnet
				if (message.equals("spieler" + 1)) { spieleranzeige.setText("Spieler 1"); }
				...
				// Die Spieler, die bereit sind werden hier aktualisiert & es wird überprüft, ob die Rohstoffkarten ausgewürfelt werden
	...
	public boolean connectToServer() {
		...
		try {
			host = new Socket(ip, 5555);
			System.out.println("Host gestartet!");
			// Streams
			writer = new PrintWriter(host.getOutputStream());
